...
So I added a jitter line to uncluster these data points which does make for a busier plot but now it is clearer what the data points are.
```{r, echo=FALSE, warning=FALSE, message=FALSE}
Freq1 %>%
arrange(Continent, Year, Freq) %>%
  ggplot(aes(x=Year, y=Freq)) +
  geom_point(mapping= aes(colour= Continent), alpha=0.5, size=2) +
  geom_smooth(aes(colour= Continent)) +
  geom_jitter(position = position_jitter(width = 0.5),alpha = .15) +
  labs(x="Years", 
          y="Number of Countries",
          title="Number of countries with a median\nlife expectancy below 40")
``` 
